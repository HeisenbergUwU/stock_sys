...
class StockCodeMap(Base):
    ...
    code: Mapped[str] = mapped_column(
        ...
        nullable=False,
        comment="股票/指数代码，如 sh.000001",
    )
    trade_status: Mapped[int] = mapped_column(
        Integer,
        nullable=False,
        server_default=text("'1'"),
        comment="交易状态：1=正常交易，0=停牌等",
    )
    code_name: Mapped[str] = mapped_column(
        String(100, "utf8mb4_unicode_ci"),
        nullable=False,
        comment="中文名称，如 上证综合指数",
    )
    is_exponent: Mapped[int] = mapped_column(
        Integer, nullable=False, server_default=text("'0'"), comment="是否为指数"
    )
...
class StockDataMonthly(Base):
    __tablename__ = "stock_data_monthly"
    __table_args__ = (
        Index("idx_code_date", "code", "date"),
        Index("uk_date_code", "date", "code", unique=True),
    )
